...
echo "=== Play Integrity Fix Waydroid Integration ==="
...
# 2. Retrieve latest Play Integrity Fix release info
echo "Fetching latest Play Integrity Fix release from GitHub..."
RELEASE_JSON=$(curl -s https://api.github.com/repos/osm0sis/PlayIntegrityFork/releases/latest)
DOWNLOAD_URL=$(echo "$RELEASE_JSON" | grep "browser_download_url" | cut -d '"' -f 4 | grep -i '.zip' | head -n 1)

if [ -z "$DOWNLOAD_URL" ]; then
    echo "Error: Could not retrieve download URL for Play Integrity Fix."
    echo "Please check your internet connection or GitHub API limits."
    exit 1
fi

VERSION=$(echo "$RELEASE_JSON" | grep '"tag_name":' | cut -d '"' -f 4)
echo "Found Play Integrity Fix version: $VERSION"

# 3. Download Play Integrity Fix
TMP_ZIP=$(mktemp -t play_integrity_XXXXXX.zip)
echo "Downloading Play Integrity Fix to temporary location: $TMP_ZIP"
curl -L -o "$TMP_ZIP" "$DOWNLOAD_URL"

if [ ! -f "$TMP_ZIP" ] || [ ! -s "$TMP_ZIP" ]; then
    echo "Error: Failed to download Play Integrity Fix."
    exit 1
fi
...
sudo cp "$TMP_ZIP" "$DATA_DIR/local/tmp/play-integrity-fix.zip"
sudo chmod 644 "$DATA_DIR/local/tmp/play-integrity-fix.zip"

echo "Installing Play Integrity Fix module via Magisk..."
if ! sudo waydroid shell -- magisk --install-module /data/local/tmp/play-integrity-fix.zip; then
    ...
    rm -f "$TMP_ZIP"
    sudo rm -f "$DATA_DIR/local/tmp/play-integrity-fix.zip"
    exit 1
fi
...
echo "========================================="
echo "Play Integrity Fix integration completed successfully!"
